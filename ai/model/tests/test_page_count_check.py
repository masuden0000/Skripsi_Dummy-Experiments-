...
def _make_p(
    *,
    has_rendered_page_break: bool = False,
    has_explicit_page_break: bool = False,
    has_sect_pr: bool = False,
) -> etree._Element:
    """Buat elemen <w:p> minimal sesuai flag yang diberikan."""
    p = etree.Element(_wns("p"))

    if has_rendered_page_break:
...
def _make_mock_para(
    *,
    ...
    para = MagicMock()
    para._p = _make_p(
        has_rendered_page_break=has_rendered_page_break,
        has_explicit_page_break=has_explicit_page_break,
        has_sect_pr=has_sect_pr,
    )
    style = MagicMock()
...
class TestCountPagesStructural:
    ...
    def test_rendered_page_break_increments_page(self):
        paras = [
            _make_mock_para(),                                     # page 1
            _make_mock_para(has_rendered_page_break=True),         # page 2
    ...
    def test_explicit_page_break_increments_page(self):
        paras = [
            _make_mock_para(),                                     # page 1
            _make_mock_para(has_explicit_page_break=True),         # page 2
            _make_mock_para(),                                     # page 2
        ]
        doc = _make_doc(paras)
        result = _count_pages_structural(doc)
    ...
    def test_sect_pr_increments_next_para(self):
        paras = [
            _make_mock_para(has_sect_pr=True),   # page 1, increments for next
            _make_mock_para(),                   # page 2
        ]
        doc = _make_doc(paras)
        result = _count_pages_structural(doc)
    ...
    def test_multiple_breaks_accumulate(self):
        paras = [
            _make_mock_para(),                                     # page 1
            _make_mock_para(has_rendered_page_break=True),         # page 2
            _make_mock_para(has_rendered_page_break=True),         # page 3
    ...
    def test_fallback_to_explicit_break_when_no_rendered_breaks(self):
        paras = [
            _make_mock_para(),                                     # page 1
            _make_mock_para(has_explicit_page_break=True),         # page 2
            _make_mock_para(),                                     # page 2
    ...
    def test_first_para_never_increments(self):
        paras = [_make_mock_para(has_rendered_page_break=True)]
        doc = _make_doc(paras)
        result = _count_pages_structural(doc)
        assert result[0] == 1
...
class TestCheckPageCount:
    ...
    @pytest.mark.skipif(not _DOCX.exists(), reason="Dokumen uji tidak tersedia")
    def test_real_docx_counts_explicit_break_before_rendered_break_heading(self):
        """Regression guard: explicit page break di paragraf sebelum heading yang
        punya rendered page break harus dihitung sebagai 2 transisi halaman terpisah.
        Pada dokumen PKM-KC test: BAB 1 = halaman 1, DAFTAR PUSTAKA = halaman 10
        → halaman inti = 10 (inklusif, sesuai aturan PKM).
        """
        ...
        start_idx, _ = _find_section_para_idx(para_list, "bab")
        end_idx, _ = _find_section_para_idx(
            para_list, "daftar_pustaka", search_from=(start_idx or 0) + 1
        )
        assert start_idx is not None and end_idx is not None
        assert page_map.get(start_idx) == 1, "BAB 1 harus di halaman 1"
        assert page_map.get(end_idx) == 10, (
            "DAFTAR PUSTAKA harus di halaman 10 sesuai header Word "
            "(explicit break di paragraf sebelumnya dan rendered break di heading "
            "harus masing-masing dihitung sebagai transisi halaman tersendiri)"
        )
        ...
        for c in checks:
            if c.status in ("passed", "failed"):
                assert c.actual == "10 halaman", (
                    f"Halaman inti seharusnya 10 (inklusif halaman DAFTAR PUSTAKA), bukan {c.actual}"
                )
