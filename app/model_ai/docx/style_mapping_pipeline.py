...
DEFAULT_EXTRACTED_PATH = DATA_DIR / "output.json"
...
# ---------------------------------------------------------------------------
# Digunakan oleh: run_docx_style_mapping_pipeline()
# Menjalankan fungsi `load_extracted_payload` sebagai bagian alur `style_mapping_pipeline`.
# ---------------------------------------------------------------------------
def load_extracted_payload(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise FileNotFoundError(f"File ekstraksi tidak ditemukan: {path}")
...
def run_docx_style_mapping_pipeline(
    ...
    extracted_path: Path = DEFAULT_EXTRACTED_PATH,
    with_embeddings: bool = True,
