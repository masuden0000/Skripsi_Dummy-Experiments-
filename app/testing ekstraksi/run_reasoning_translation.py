...
def _detect_input_kind(payload: Any) -> str:
    if isinstance(payload, dict):
        return "output"
    raise ValueError("Pipeline reasoning hanya menerima object output.json, bukan output_chunks.json.")
...
def _build_source_facts(payload: Any, input_kind: str) -> list[SourceFact]:
    if not isinstance(payload, dict):
        raise ValueError("input-kind output membutuhkan JSON object dari output.json.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Testing ekstraksi: reasoning audit output.json ke properti python-docx."
    )
